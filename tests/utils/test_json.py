...
from xTool import misc as utils_json
...
class TestNumpyJsonEncoder(unittest.TestCase):
    ...
    def test_encode_datetime(self):
        obj = datetime.strptime('2017-05-21 00:00:00', '%Y-%m-%d %H:%M:%S')
        self.assertEqual(
            json.dumps(obj, cls=utils_json.NumpyJsonEncoder),
            '"2017-05-21T00:00:00Z"'
        )

    def test_encode_date(self):
        self.assertEqual(
            json.dumps(date(2017, 5, 21), cls=utils_json.NumpyJsonEncoder),
            '"2017-05-21"'
        )

    def test_encode_numpy_int(self):
        self.assertEqual(
            json.dumps(np.int32(5), cls=utils_json.NumpyJsonEncoder),
            '5'
        )

    def test_encode_numpy_bool(self):
        self.assertEqual(
            json.dumps(np.bool_(True), cls=utils_json.NumpyJsonEncoder),
            'true'
        )

    def test_encode_numpy_float(self):
        self.assertEqual(
            json.dumps(np.float16(3.76953125), cls=utils_json.NumpyJsonEncoder),
            '3.76953125'
        )
    ...
    def test_encode_raises(self):
        self.assertRaisesRegexp(TypeError,
                                ...
                                json.dumps,
                                Exception,
                                cls=utils_json.NumpyJsonEncoder)
